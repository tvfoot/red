 ...
 */
package io.oldering.tvfoot.red.util.plaid;
...
public class ReflowText extends Transition {
  ...
  @Override public Animator createAnimator(ViewGroup sceneRoot, TransitionValues startValues,
    ...
    final Bitmap startText =
        createBitmap(startData, startLayoutMaxLines != null ? startLayoutMaxLines : startLayout);
    ...
    List<Run> runs =
        getRuns(startData, startLayout, startLayoutMaxLines, endData, endLayout, endLayoutMaxLines);
  ...
  @Override public Transition setDuration(long duration) {
  ...
  private List<Run> getRuns(@NonNull ReflowData startData, @NonNull Layout startLayout,
      @Nullable Layout startLayoutMaxLines, @NonNull ReflowData endData, @NonNull Layout endLayout,
      @Nullable Layout endLayoutMaxLines) {
    ...
    for (int i = 0; i < textLength; i++) {
      ...
      if (startLine != currentStartLine || endLine != currentEndLine || lastChar) {
        // at a run boundary, store bounds in both states
        int startRunRight =
            getRunRight(startLayout, startLayoutMaxLines, currentStartLine, i, startLine, startMax,
                startMaxEllipsis, lastChar);
        int startRunBottom = startLayout.getLineBottom(currentStartLine);
        int endRunRight =
            getRunRight(endLayout, endLayoutMaxLines, currentEndLine, i, endLine, endMax,
                endMaxEllipsis, lastChar);
        int endRunBottom = endLayout.getLineBottom(currentEndLine);

        Rect startBound =
            new Rect(currentStartRunLeft, currentStartRunTop, startRunRight, startRunBottom);
        startBound.offset(startData.textPosition.x, startData.textPosition.y);
        Rect endBound = new Rect(currentEndRunLeft, currentEndRunTop, endRunRight, endRunBottom);
        endBound.offset(endData.textPosition.x, endData.textPosition.y);
        runs.add(new Run(startBound, startMax || startRunBottom <= startData.textHeight, endBound,
            endMax || endRunBottom <= endData.textHeight));
        currentStartLine = startLine;
        currentStartRunLeft = (int) (startMax ? startLayoutMaxLines.getPrimaryHorizontal(i)
            : startLayout.getPrimaryHorizontal(i));
        currentStartRunTop = startLayout.getLineTop(startLine);
        currentEndLine = endLine;
        currentEndRunLeft = (int) (endMax ? endLayoutMaxLines.getPrimaryHorizontal(i)
            : endLayout.getPrimaryHorizontal(i));
   ...
   */
  private int getRunRight(Layout unrestrictedLayout, Layout maxLinesLayout, int currentLine,
      int index, int line, boolean withinMax, boolean isMaxEllipsis, boolean isLastChar) {
    int runRight;
   ...
   */
  @NonNull private List<Animator> createRunAnimators(View view, ReflowData startData,
      ReflowData endData, Bitmap startText, Bitmap endText, List<Run> runs) {
    ...
    for (int i = upward ? 0 : runs.size() - 1; ((upward && i < runs.size()) || (!upward && i >= 0));
        i += (upward ? 1 : -1)) {
      Run run = runs.get(i);
      ...
      SwitchDrawable drawable =
          new SwitchDrawable(startText, run.start, startData.textSize, endText, run.end,
              endData.textSize);
      ...
      PropertyValuesHolder topLeft = PropertyValuesHolder.ofObject(SwitchDrawable.TOP_LEFT, null,
          getPathMotion().getPath(run.start.left + dx, run.start.top + dy, run.end.left,
              run.end.top));
      PropertyValuesHolder width =
          PropertyValuesHolder.ofInt(SwitchDrawable.WIDTH, run.start.width(), run.end.width());
      PropertyValuesHolder height =
          PropertyValuesHolder.ofInt(SwitchDrawable.HEIGHT, run.start.height(), run.end.height());
      ...
      Animator runAnim =
          ObjectAnimator.ofPropertyValuesHolder(drawable, topLeft, width, height, progress);
      ...
      if (run.startVisible != run.endVisible) {
        ...
        ObjectAnimator fade =
            ObjectAnimator.ofInt(drawable, SwitchDrawable.ALPHA, OPAQUE, OPACITY_MID_TRANSITION,
                OPAQUE);
        fade.setStartDelay(startDelay);
  ...
  private Layout createLayout(ReflowData data, Context context, boolean enforceMaxLines) {
    ...
    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
      StaticLayout.Builder builder =
          StaticLayout.Builder.obtain(data.text, 0, data.text.length(), paint, data.textWidth)
  ...
  private Bitmap createBitmap(@NonNull ReflowData data, @NonNull Layout layout) {
    Bitmap bitmap =
        Bitmap.createBitmap(data.bounds.width(), data.bounds.height(), Bitmap.Config.ARGB_8888);
  ...
  private static class ReflowData implements Parcelable {
    ...
    @SuppressWarnings("unused") public static final Parcelable.Creator<ReflowData> CREATOR =
        new Parcelable.Creator<ReflowData>() {
          @Override public ReflowData createFromParcel(Parcel in) {
            return new ReflowData(in);
          }

          @Override public ReflowData[] newArray(int size) {
  ...
  private static class SwitchDrawable extends Drawable {
    ...
    static final Property<SwitchDrawable, PointF> TOP_LEFT =
        new Property<SwitchDrawable, PointF>(PointF.class, "topLeft") {
          @Override public void set(SwitchDrawable drawable, PointF topLeft) {
            drawable.setTopLeft(topLeft);
          }

          @Override public PointF get(SwitchDrawable drawable) {
    ...
    static final Property<SwitchDrawable, Integer> WIDTH =
        new Property<SwitchDrawable, Integer>(Integer.class, "width") {
          @Override public void set(SwitchDrawable drawable, Integer width) {
            drawable.setWidth(width);
          }

          @Override public Integer get(SwitchDrawable drawable) {
    ...
    static final Property<SwitchDrawable, Integer> HEIGHT =
        new Property<SwitchDrawable, Integer>(Integer.class, "height") {
          @Override public void set(SwitchDrawable drawable, Integer height) {
    ...
    static final Property<SwitchDrawable, Integer> ALPHA =
        new Property<SwitchDrawable, Integer>(Integer.class, "alpha") {
          @Override public void set(SwitchDrawable drawable, Integer alpha) {
    ...
    static final Property<SwitchDrawable, Float> PROGRESS =
        new Property<SwitchDrawable, Float>(Float.class, "progress") {
          @Override public void set(SwitchDrawable drawable, Float progress) {
            drawable.setProgress(progress);
          }

          @Override public Float get(SwitchDrawable drawable) {
            return 0f;
          }
        };
    ...
    SwitchDrawable(@NonNull Bitmap startBitmap, @NonNull Rect startBitmapSrcBounds,
        float startFontSize, @NonNull Bitmap endBitmap, @NonNull Rect endBitmapSrcBounds,
        float endFontSize) {
  ...
  public static class ReflowableTextView implements Reflowable<TextView> {
    ...
    @Override public TextView getView() {
      return textView;
    }
    ...
    @Override public Point getTextPosition() {
    ...
    @Override public float getLineSpacingAdd() {
      return textView.getLineSpacingExtra();
    }

    @Override public float getLineSpacingMult() {
      return textView.getLineSpacingMultiplier();
    }

    @Override public int getBreakStrategy() {
      if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
    ...
    @Nullable @Override public String getFontName() {
