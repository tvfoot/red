...
public class MatchListViewModelUnitTest {
  ...
  @Before public void before() throws IOException {
    ...
    component =
        DaggerTestComponent.builder().networkModule(new NetworkModule(baseUrl.toString())).build();
    schedulerProvider = component.schedulerProvider();
  }

  @Test public void findFuture() throws IOException {
  ...
  @Test public void groupByDate() throws IOException {
    // setup
    Fixture fixture = component.fixture();
    MatchListViewModel matchListViewModel = component.matchListViewModel();
    ...
    // execute
    Observable<GroupedObservable<String, Item>> groupedMatchesObservable =
        matchListViewModel.groupByDate(matches);

    // verifty
    List<List<Item>> expectedGroupedObservables =
        createGroupedObservableItems(component, matchesAsList);
    ...
    List<GroupedObservable<String, Item>> groupedObservables =
        groupedMatchesObservable.toList().blockingGet();
  ...
  @Test public void insertDayHeader() throws IOException {
    // setup
    Fixture fixture = component.fixture();
    MatchListViewModel matchListViewModel = component.matchListViewModel();
    ...
    // execute
    Observable<Observable<Item>> withDayHeadersGroupedMatches =
        matchListViewModel.insertDayHeader(groupedMatchesObservable);

    // verify
    List<List<Item>> expectedItemsObservables =
        createItemObservablesWithHeader(component, matchesAsList);
  ...
  private List<List<Item>> createItemObservablesWithHeader(TestComponent component,
      List<Match> matchesAsList) {
  ...
  private List<List<Item>> createGroupedObservableItems(TestComponent component,
      List<Match> matchesAsList) {
    List<List<Item>> matchItemsAsList = new ArrayList<>();
  ...
  @Test public void getFilter() {
    ...
    String filter =
        "{\"where\":{\"deleted\":{\"neq\":1}},\"order\":\"start-at ASC, weight ASC\",\"limit\":30,\"offset\":30}";
